...
<!-- pw-contents:begin -->
## Contents   [🤖-owned — regenerated by `pw-lib.sh review reindex`; never hand-edit]
...
- **The `## Contents` table above is 🤖-owned** — heading-text-anchored (never a line number, so
  it never goes stale on a rewrite), regenerated by `pw-lib.sh review reindex <slug> <this-file>`
...
- **Once several items are `[RESOLVED]`/`[ANSWERED]`, run `pw-lib.sh review archive <slug>
  ...
  `[OPEN]`/`[PENDING]` headings or the Sign-off table — see docs/REVIEW.md. Keeps a long-lived
