...
`task/T0n.md`→`task/review/T0n.review.md` (from `base/_REVIEW.template.md`). Rules you MUST follow:
...
- **Scaffolded project dirs** (`$PW_PROJECTS/<slug>/`) are **plain (not git repos)** by design —
  ...
  (no `workspace-` bucket for them). The reusable **`base/` bundle itself IS a git repo** (so it's
...
`base/` templates or this skill, set `Status: done`, and summarize MRs/leftovers. Don't save what
