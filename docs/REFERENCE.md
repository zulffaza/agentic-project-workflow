...
│                                   EXECUTION · REFERENCE · RFC · MEMORY · TOOLING)
...
(There is one more directory, the machinery the commands call under the hood. You don't operate it,
edit it, or need it day-to-day — its story is [TOOLING.md](./TOOLING.md).)
...
Reusable sub-agents are **not** per-project — the installer seeds them globally
...
| `/pw-verify <target-of-verification>` | independent fresh-context `pw-verifier` stand-alone check — not part of the default executor flow; lands findings as review items, never edits |
...
reports what drifted — you never run a generator yourself). Enable/disable or add a provider → edit
[`pw.config.sh`](../pw.config.example.sh). Curious how the machinery is laid out?
[TOOLING.md](./TOOLING.md) and [ONBOARDING.md](../ONBOARDING.md).
