...
`/pw-rfc <slug> comments` only **pulls**: it calls the backend's `list_comments` and writes each
open thread into `analysis/review/RFC.review.md` (create it via `pw-lib.sh review-init` if absent),
one item per thread — quote the comment text, name the author, link the thread, and note which
canonical section it's anchored to. The agent **never replies or resolves on the external
platform** — you do that yourself, using the local record as your reference. You then run the
**existing** `/pw-review` to apply fixes to the underlying analysis locally; its `↳ agent:` reply on
each item is the durable record of what changed to address it (same mechanics as every other
review file — no new review machinery). Re-run `/pw-rfc <slug>` afterward to push the revised
sections. This mirrors `docs/REVIEW.md`'s MR-comment flow's FETCH → MIRROR steps, but deliberately
**drops** its FIX-in-worktree/REPLY-on-thread steps — those are code-review mechanics that don't
apply to RFC prose, and you explicitly own the on-platform reply here. No-ops with a clear message
under the `markdown` backend (nothing external to pull).
...
- **`rfc/META.md`** — 🤖-owned via `pw-lib.sh rfc init|target|state` (never hand-edit): backend,
  target ref, last revision pushed, wave1/wave2-published flags, comment-sync cursor (dedup, same
  spirit as MR-comment dedup). `rfc init` stamps `Backend:` with the **actual** resolved backend
  it's called with — never a hardcoded guess — so this file is correct from the moment it exists.
