...
| _`<future>`_ | _stub or implemented_ | _…_ | _…_ | _…_ | _…_ | Maintainer implements it and adds a row — no code change needed, but see "Adding a backend" below. |
...
No `/pw-rfc` command-logic change is required once a maintainer adds a row — the command reads
this file at publish time.
