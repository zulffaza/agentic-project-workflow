...
pwtest_rc 0 "review gate plan lane F2 approved" "$TOOL/pw-preflight.sh" review "$E" plan
pwtest_rc 1 "bogus mode rc1 + actionable usage" "$TOOL/pw-preflight.sh" bogus "$E"
...
pwtest_rc 1 "analyze F1 scaffold: example-only INDEX is NO input" "$TOOL/pw-preflight.sh" analyze "$S1"
...
pwtest_rc 0 "analyze passes with one REAL context row" "$TOOL/pw-preflight.sh" analyze "$A"
rm -rf "$PW_PROJECTS_DIR/$A"
pwtest_rc 1 "analyze after phase moved on names --rewind" "$TOOL/pw-preflight.sh" analyze "$S2"
...
pwtest_rc 0 "ship gate F2 finds shippable" "$TOOL/pw-preflight.sh" ship "$E"
# close gate on F2 (nothing accepted): refuse, must name the offenders (close semantics):
"$TOOL/pw-lib.sh" status "$E" review >/dev/null 2>&1 || true
pwtest_rc 1 "close gate refuses with offenders named" "$TOOL/pw-preflight.sh" close "$E"
