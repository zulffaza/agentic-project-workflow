...
pwtest_rc 2 "no rfc dir → missing-setup error" "$(pwtest_script pw-rfc-comments.sh)" "$R"
pwtest_fix "names a next command"
rm -rf "$PW_PROJECTS_DIR/$R"
mkdir -p "$PW_PROJECTS_DIR/${S2}rfc-copy" 2>/dev/null; true
pwtest_rc any "unknown backend rejected" env PW_RFC_BACKEND=nosuchbackend "$(pwtest_script pw-rfc-comments.sh)" "$S2"
