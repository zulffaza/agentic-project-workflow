...
pwtest_rc 0 "resolve F2 rows" "$(pwtest_script pw-ship-resolve.sh)" "$S2"
...
pwtest_rc 0 "resolve F3 hostile (plain id + linked id + quotes)" "$(pwtest_script pw-ship-resolve.sh)" "$S3"
pwtest_re '^T01\|' "plain id row parsed (C4)"
pwtest_rc 2 "resolve no PLAN → 2" "$(pwtest_script pw-ship-resolve.sh)" "$S1"
