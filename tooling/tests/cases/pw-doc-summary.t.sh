...
pwtest_rc 0 "summary plan F2" "$TOOL/pw-doc-summary.sh" plan "$S2"
...
pwtest_rc 0 "summary plan F3 hostile" "$TOOL/pw-doc-summary.sh" plan "$S3"
pwtest_re '^Tasks: 6' "hostile rows (plain+linked ids, quotes) parsed"
pwtest_rc 0 "summary project F2" "$TOOL/pw-doc-summary.sh" project "$S2"
pwtest_re '^Phase: executing' "phase token extracted (not prose)"
pwtest_rc 0 "summary project F3" "$TOOL/pw-doc-summary.sh" project "$S3"
pwtest_rc 0 "summary task F2 T01" "$TOOL/pw-doc-summary.sh" task "$S2" T01
pwtest_re 'Repo: api' "task keys parsed"
pwtest_rc 2 "unknown project" "$TOOL/pw-doc-summary.sh" project nope
