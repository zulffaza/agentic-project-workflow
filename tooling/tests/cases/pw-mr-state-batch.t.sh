...
pwtest_rc 0 "batch F2 every task" "$TOOL/pw-mr-state-batch.sh" "$S2"
pwtest_re 'T02\|closed' "closed state mapped (glab vocab)"
: > "$PWTEST_FORGE_STATE_FILE"; printf "42 merged\n43 opened\n" >> "$PWTEST_FORGE_STATE_FILE"
: > "$PWTEST_FORGE_LOG"
pwtest_rc 0 "batch F3 T02+T04 (linked cell + decoy)" "$TOOL/pw-mr-state-batch.sh" "$S3" T02 T04
...
: > "$PWTEST_FORGE_LOG"
pwtest_rc 0 "batch F3 auto-list over linked cells" "$TOOL/pw-mr-state-batch.sh" "$S3"
...
_pwt_lib_resolve() {
  eval "$(sed -n '/^_resolve_task_mr_url()/,/^}/p' "$TOOL/pw-lib.sh")"
