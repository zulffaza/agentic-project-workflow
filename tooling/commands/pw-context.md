...
- **`/pw-context <slug> req-init`** → `{{PW_HOME}}/tooling/pw-context.sh req-init <slug>` —
...
- **`/pw-context <slug> add-input --file <f> --what <prose…> --source <prose…> [--trust <prose…>]`**
  → pass the flags through to `{{PW_HOME}}/tooling/pw-context.sh add-input <slug> …` exactly as
...
- **`/pw-context <slug> add-repo <repo> <base> <why…>`** → `{{PW_HOME}}/tooling/pw-context.sh
